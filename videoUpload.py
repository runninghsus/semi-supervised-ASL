...
def load_view():
    ...
    idx = 0
    handmark_names = [e.name for e in mp_hands.HandLandmark]
            ...
            with mp_hands.Hands(
              ...
              if ret:
                  ...
                  for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    # )
                    # if idx % 5 == 0:
                    # for landmark in handmark_names:
                    for i in range(len(hand_landmarks.landmark)):
                        # hand_landmarks.landmark[mp_hands.HandLandmark.name[0]]
                        positionX = st.write(hand_landmarks.landmark[i].x * image_width)
                        positionY = st.write(hand_landmarks.landmark[i].y * image_height)
                    ...
                    with open('Database.csv','a', newline='') as f:
                        ...
                        ax.scatter(idx, x, y, c='pink')
                        
                        x2 = hand_landmarks.landmark[mp_hands.HandLandmark.WRIST].x * image_width
                        y2 = hand_landmarks.landmark[mp_hands.HandLandmark.WRIST].y * image_height
                        ax.scatter(idx, x2, y2, c='blue')

                        x3 = hand_landmarks.landmark[mp_hands.HandLandmark.MIDDLE_FINGER_TIP].x * image_width
                        y3 = hand_landmarks.landmark[mp_hands.HandLandmark.MIDDLE_FINGER_TIP].y * image_height
                        ax.scatter(idx, x3, y3, c='purple')
